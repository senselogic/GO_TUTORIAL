...
func ( model * MODEL ) GetArticleById( article * ARTICLE, id uint32 ) {
...
func ( model * MODEL ) GetArticleArrayBySectionId( article_array * [] ARTICLE, section_id uint32 ) {
...
func ( model * MODEL ) ChangeArticle( id uint32, title string, text string, image string, date string, section_id uint32, user_id uint32 ) {
    database.MustExec( "update ARTICLE set Id = ?, Title = ?, Text = ?, Image = ?, Date = ?, SectionId = ?, UserId = ? where Id = ?", id, title, text, image, date, section_id, user_id, id );
}

// ~~

func ( model * MODEL ) AddArticle( title string, text string, image string, section_id uint32, user_id uint32 ) {
    database.MustExec( "insert into ARTICLE ( Title, Text, Image, Date, SectionId, UserId ) values ( ?, ?, ?, NOW(), ?, ? )", title, text, image, section_id, user_id, );
}

// ~~

func ( model * MODEL ) RemoveArticle( id uint32 ) {
