...
func ( model * MODEL ) GetCommentById( comment * COMMENT, id uint32 ) {
...
func ( model * MODEL ) GetCommentArrayByArticleId( comment_array * [] COMMENT, article_id uint32 ) {
...
func ( model * MODEL ) ChangeComment( id uint32, text string, date string, article_id uint32, user_id uint32 ) {
    database.MustExec( "update COMMENT set Id = ?, Text = ?, Date = ?, ArticleId = ?, UserId = ? where Id = ?", id, text, date, article_id, user_id, id );
}

// ~~

func ( model * MODEL ) AddComment( text string, article_id uint32, user_id uint32 ) {
    database.MustExec( "insert into COMMENT ( Text, Date, ArticleId, UserId ) values ( ?, NOW(), ?, ? )", text, article_id, user_id );
}

// ~~

func ( model * MODEL ) RemoveComment( id uint32 ) {
